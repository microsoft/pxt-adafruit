...
Instead of continually reading the value of a pin input, you can have code to run when an input level changes on a pin. These are pin events and they are triggered when an input switches from one level to another or if the input level is beginning to change.
...
Data is transmitted between digital circuits using changes between high and low voltage levels. A change to high can mean a signal for the value of `1`. The change down to low can mean a signal for the value of `0`. One of these signals can represent a binary digit, or _bit_. Combining several digital signals will form a binary number which can represent some type of data.
...
Another way to signal data is to keep the signal level constant while the a value represented is the same. More values  might be read by pin **A1** in the same amount of time.
...
Timing events is an important part of digital electronics. Actions in a computer are sequenced by a system clock. This clock is actually a digital signal using continuous pulses that occur at a regular interval. In fact, each part of a program on your @boardname@ is run in the microcontoller at a certain time that's controlled by a clock signal. A clock signal at pin **A2** is shown in the following diagram.
...
If we want to code an event to read digital numbers from a pin (as discussed in [Digital data signals](/learnsystem/pins-tutorial/digital-input/pin-events#data-signals)), we could put it in a ``||pins:on pin pulse high||`` event. We'll code the event for the clock signal on pin **A2** and the data signal is read from **A1**. The bits are accumulated until an 8 bit number is formed. Then, each 8 bit number is stored in a data array.
...
The pulse low event can be used for a negative logic signal. For example, a pushbutton or keyboard switch could ground a pin input (set the pin voltage to **GND**) to signal a key press. You could code a key press event like this:
...
When a pin is receiving pulse inputs, an ``||pins:on pin rise||`` will trigger at the beginning of the pulse. This is useful for detecting the next period of a clock signal input. If using a pulse event, only the time of the pulse with is detected. That was 2 microseconds in our previous example. If a ``rise`` event is used, the event will trigger only at the beginning of each clock cycle which is 4 microseconds long.

Now with the digital number input example, instead of using a pulse event, we can use a rise event to read in our numbers:
